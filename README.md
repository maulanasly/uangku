...
- **Analytics** — filter spending by date range (All time, Last 30 days, Last 90 days, Last 6 months, This year, or a custom picker; defaults to Last 30 days). Short ranges render a day-by-day spending trend; longer ranges aggregate by month. Includes a category donut chart (tap a slice to highlight it), a current-month "budget vs. spent" comparison (the budget line is drawn on the trend chart only when the range is the current lone month), top items per category, a full category breakdown, and a category trend comparing the selected period against the preceding period of equal length
